...
export default function BlogClientContent({ initialPosts, categories }) {
  const searchParams = useSearchParams();
  const activeCategory = searchParams.get("category");

  // Filter posts client-side for zero-latency switching
  ...
  return (
    <div className="py-24 bg-dark-bg min-h-screen relative overflow-hidden bg-premium-mesh">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Category tabs using real links (SEO-friendly) with scroll={false} for soft route hydration */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {filteredPosts.length === 0 && (
            <div className="col-span-1 md:col-span-3 pb-20 pt-10 text-center flex flex-col items-center justify-center">
               <p className="text-gray-400 font-medium tracking-wide">No proprietary intel found in this vertical.</p>
            </div>
          )}
        </div>
      </div>
