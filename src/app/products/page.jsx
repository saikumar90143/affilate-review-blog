...
export default async function ProductsPage({ searchParams }) {
  ...
  try {
    // Fetch product-specific categories only
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {categories.length > 0 && (
          <div className="flex justify-start md:justify-center gap-2 md:gap-3 mb-10 md:mb-16 overflow-x-auto pb-4 px-2 md:px-0 snap-x">
            <Link href="/products" className={!activeCategory ? activeTabStyle : inactiveTabStyle}>
              All Products
            </Link>
            {categories.map(cat => (
              <Link
        ...
        {products.length === 0 ? (
          <div className="text-center py-24 text-gray-500">
            ...
            <p className="text-sm mt-2">Add products from the Admin Dashboard to get started.</p>
          </div>
        ) : (
